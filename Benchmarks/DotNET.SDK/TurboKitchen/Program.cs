...
namespace Kitchen
{
    class Program
    {
        ...
        static async Task DoLoop(Client client)
        {
            ...
            while (true)
            {
                foreach (var tenant in GetTenants())
                {
                    ...
                    var taco = new DishPrepared($"Taco {i} med lefse till {tenant}", "Mrs. Turbo Taco, with love", DateTime.Parse("2021-01-11T06:51:57.181Z"));
        ...
        static void CreateEventHandlers(EventHandlersBuilder builder, int amount)
        {
            ...
            {
                builder
                    .CreateEventHandler("d786e18f-c227-456c-8f9d-bee0a8784134")
                    .Partitioned()
